...
public class Piece {
    ...
    public void pawnPromotion() { //checks for pawn promotion to new piece
        Scanner input = new Scanner(System.in);
        if (character == '\u2659' || character == '\u265f') {
            if (row == 0 || row == 7) { //checks if a pawn has made it to the edge of the board
                System.out.println("Your pawn can be promoted. Chose promotion character: 'Q' (Queen), 'R' (Rook), 'K' (Knight), 'B' (Bishop)");
                String promoCharacter = input.nextLine();
                if (promoCharacter.equals("Q"))
                    if (character == '\u2659')
                        this.character = '\u2655';
                    else
                        this.character = '\u265b';
                else if (promoCharacter.equals("R"))
                    if (character == '\u2659')
                        this.character = '\u2656';
                    else
                        this.character = '\u265c';
                else if (promoCharacter.equals("K"))
                    if (character == '\u2659')
                        this.character = '\u2658';
                    else
                        this.character = '\u265e';
                else if (promoCharacter.equals("B"))
                    if (character == '\u2659')
                        this.character = '\u2657';
                    else
                        this.character = '\u265d';
